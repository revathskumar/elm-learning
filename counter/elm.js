...
Elm.Main.make = function (_elm) {
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([]),_U.list([$Html.text("Counter")]))
              ,A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"From model :: ",$Basics.toString(model)))]))
              ,A2($Html.div,
              _U.list([]),
              _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,address,Dec)]),_U.list([$Html.text("-")]))
                      ,A2($Html.span,_U.list([]),_U.list([$Html.text($Basics.toString(model))]))
                      ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Inc)]),_U.list([$Html.text("+")]))]))]));
   });
   var NoOp = {ctor: "NoOp"};
   var actions = $Signal.mailbox(NoOp);
   var model = A3($Signal.foldp,update,0,actions.signal);
   var main = A2($Signal.map,view(actions.address),model);
   var initModel = 0;
   return _elm.Main.values = {_op: _op,initModel: initModel,NoOp: NoOp,Inc: Inc,Dec: Dec,update: update,view: view,actions: actions,model: model,main: main};
};
