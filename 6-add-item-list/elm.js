...
Elm.Main.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var todoList = function (item) {    return A2($Html.li,_U.list([]),_U.list([$Html.text(item)]));};
   var UpdateField = function (a) {    return {ctor: "UpdateField",_0: a};};
   var Add = {ctor: "Add"};
   var view = F2(function (address,todos) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Add Item")]))
              ,A2($Html.div,
              _U.list([]),
              _U.list([A2($Html.input,
                      _U.list([$Html$Attributes.type$("text")
                              ,$Html$Attributes.name("list")
                              ,$Html$Attributes.value(todos.field)
                              ,$Html$Attributes.placeholder("Todo")
                              ,A3($Html$Events.on,"input",$Html$Events.targetValue,function (_p0) {    return A2($Signal.message,address,UpdateField(_p0));})]),
                      _U.list([]))
                      ,A2($Html.button,_U.list([$Html$Attributes.name("add"),A2($Html$Events.onClick,address,Add)]),_U.list([$Html.text("Add Todo")]))]))
              ,A2($Html.div,_U.list([]),_U.list([A2($Html.ul,_U.list([]),A2($List.map,todoList,todos.items))]))]));
   });
   var NoOp = {ctor: "NoOp"};
   var actions = $Signal.mailbox(NoOp);
   var addItem = function (todos) {    return _U.update(todos,{items: A2($List.append,todos.items,_U.list([todos.field])),field: ""});};
   var update = F2(function (action,todos) {
      var _p1 = action;
      switch (_p1.ctor)
      {case "NoOp": return todos;
         case "Add": return addItem(todos);
         default: return _U.update(todos,{field: _p1._0});}
   });
   var initTodos = {items: _U.list([]),field: ""};
   var model = A3($Signal.foldp,update,initTodos,actions.signal);
   var main = A2($Signal.map,view(actions.address),model);
   var Todos = F2(function (a,b) {    return {items: a,field: b};});
