...
Elm.Main.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var update = F2(function (action,user) {    var _p0 = action;if (_p0.ctor === "NoOp") {    return user;} else {    return _p0._0;}});
   var UpdateName = function (a) {    return {ctor: "UpdateName",_0: a};};
   var view = F2(function (address,user) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,
              _U.list([]),
              _U.list([A2($Html.span,_U.list([]),_U.list([$Html.text("Enter Your Name :: ")]))
                      ,A2($Html.input,
                      _U.list([$Html$Attributes.type$("text")
                              ,$Html$Attributes.name("name")
                              ,$Html$Attributes.value(user)
                              ,A3($Html$Events.on,"input",$Html$Events.targetValue,function (_p1) {    return A2($Signal.message,address,UpdateName(_p1));})]),
                      _U.list([]))]))
              ,A2($Html.div,_U.list([]),_U.list([A2($Html.span,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"My Name is ",user))]))]))]));
   });
   var NoOp = {ctor: "NoOp"};
   var actions = $Signal.mailbox(NoOp);
   var model = A3($Signal.foldp,update,"",actions.signal);
   var main = A2($Signal.map,view(actions.address),model);
   return _elm.Main.values = {_op: _op,NoOp: NoOp,UpdateName: UpdateName,view: view,update: update,actions: actions,model: model,main: main};
};
